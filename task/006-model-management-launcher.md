...
release build、`swiftc portable/launcher/app/*.swift`、`swiftc CredentialHelper.swift`、
`zsh -n` 3本、一時ディレクトリでの `build_launcher`
（plutil / icon / swiftc / codesign --verify）、実state に対する読み取り専用の
`profile show --json` と `doctor`（RESULT: PASS）。
...
- `select_profile_path` の6箇所の重複を `profile.installed_profile()` へ集約。
  supervisorだけlegacy fallbackが無くdoctorと別のprofileを選びうる不整合も解消
- 既定モデルの妥当性判定が保存可否・popup・payloadで別々だった問題（`resolvedDefault`）
- CLIがstdinを読む前に死ぬとSIGPIPEでランチャーごと落ちる問題
- CLIエラーを出すlabelが1行に潰れて原因が中略で消える問題
- 「クリックで自動更新／切替確認」というdocsとコードの食い違い

3巡目:

- 起動後のfolder dropで、隠したはずの管理画面が戻る問題
- applyがcatalogを消すのに `catalog_profile_digest` を残しており、
