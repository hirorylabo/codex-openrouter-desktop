...
- A successful save reports that the change takes effect on the next OpenRouter launch. With two or more visible models the new default is applied exactly once and the picker choice is respected afterwards. With a single visible model there is nothing else to pick, so every dedicated launch selects that model and `model_provider = "openrouter"` again, and shutdown restores the native default.
...
Tool-bearing upstream requests additionally record `tool_request`, `duration_ms`, and only the integer `input_tokens`, `output_tokens`, and `cached_tokens` the response itself reported. `duration_ms` measures the upstream round trip and does not depend on prompts, tool names, or arguments. Non-integer or unknown usage shapes are omitted rather than reinterpreted, and totals are never recomputed.
...
For manual checks with the real ChatGPT.app, run the isolated-home E2E first. After upgrading the installed runtime, run two launcher cycles; the latter command asks you to quit ChatGPT.app normally during each cycle. When passed an empty workspace, cycle 1 asks you to use the newly opened chat for five gates: one `pwd`, one `apply_patch`, one namespace-child call, then a dependent two-call turn whose second call must use the first call's read result, and a parallel turn with two mutually independent read-only commands. A read-only JSONL audit requires the exact cwd, tool, arguments, output, and final answer before the harness can continue; the parallel gate compares the calls as a set rather than in order. The value the dependent gate reads is generated by the harness into the workspace and never appears in any GUI prompt. Resuming an existing chat preserves its stored cwd, so that mismatch stops the run before cycle 2. Treat `--open-project` as a current-build internal contract rather than the acceptance signal.
